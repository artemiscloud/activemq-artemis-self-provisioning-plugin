...
export const getConditionOKCount = (
  ...
  _.sumBy<any>(conditions, (c) =>
    Number(c.status === K8sResourceConditionStatus.True),
  );

export const getConditionString = (
  conditions: K8sResourceCondition[],
): string => `${getConditionOKCount(conditions)} OK / ${_.size(conditions)}`;

export const getCondition = (
  conditions: K8sResourceCondition[],
  type: K8sResourceCondition['type'],
): K8sResourceCondition | undefined =>
  _.find(conditions, (c) => c.type === type);
